...
CONV4_KERNEL = 16
FC1_UNITS = 1024
FEATURE_DIM = 25  #10, 20, 25, 28, 30, 31, 32, 35, 40, 50, 60, 65, 70, 75, 80
LSTM_LAYER = 2 # The number of lstm layer, 1,2,3
HIDDEN_UNITS = 25  # The number of output feature dimension, 10，15，20，25，30，35，40，50，60，70，80
...
INITIAL_LEARNING_RATE = 0.02       # Initial learning rate.初始学习率
...
def inference(datasets, batch_size):
    ...
    with tf.variable_scope('fc2') as scope:
        weights = _variable_with_weight_decay('weights',shape=[1024,FEATURE_DIM],stddev=0.1,wd=0.0)
        biases  = _variable_on_cpu('biases',[FEATURE_DIM],tf.constant_initializer(0.1))
        local6 = tf.nn.relu(tf.matmul(local5,weights)+biases,name=scope.name)
        variable_summaries(weights)
        
    with tf.variable_scope('softmax_linear') as scope:
        weights = _variable_with_weight_decay('weights',shape=[FEATURE_DIM,lstm_data.NUM_CLASSES],stddev=0.1,wd=0.0)
...
def train(total_loss, global_step, decay_steps, batch_size):
...
def do_eval(sess, eval_correct, Samples_placeholder, Labels_placeholder, filename_queue, batch_size, total_size):
    ...
    for _ in range(steps_per_epoch):
        x_batch,y_batch=lstm_data.read_and_decode(filename_queue, batch_size=batch_size, shuffle_batch=False)
...
def RNN(X, batch_size):
    ...
    with tf.variable_scope('RNN_in') as scope:
        weights = _variable_with_weight_decay('weights',shape=[FEATURE_DIM,HIDDEN_UNITS],stddev=0.1,wd=0.0)
