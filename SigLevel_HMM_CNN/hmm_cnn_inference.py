...
def inference(datasets, batch_size):
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = _variable_with_weight_decay('weights',shape=[FEATURE_DIM,hmm_data.NUM_CLASSES],stddev=0.1,wd=0.0)
        biases  = _variable_on_cpu('biases',[hmm_data.NUM_CLASSES],tf.constant_initializer(0.1))
...
def do_eval(sess, eval_correct, Samples_placeholder, Labels_placeholder, filename_queue, batch_size, total_size):
    ...
    for _ in range(steps_per_epoch):
        x_batch,y_batch=hmm_data.read_and_decode(filename_queue, batch_size=batch_size, shuffle_batch=False)
...
def accuracy_score(expected, predicted):
    cm = np.array(metrics.confusion_matrix(expected, predicted + 1))
    return sum(cm.max(0))/len(predicted)